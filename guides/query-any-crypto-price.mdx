---
title: "Get any crypto price (100% coverage)"
icon: "key-skeleton"
...
Mobula API allows you to query market data from any kind of assets in a unified way. You can query any crypto price, volume, price change, and more from any blockchain supported (check [here](/api-reference/blockchains) for the full list) or asset listed on Mobula curated token list (explorable [here](https://mobula.io)).
...
- Fetch up to 500 assets at once with market/multi-data endpoint (check [here](/api-reference/endpoint/get-multiple-assets-market-data) for more details)
- Subcribe to real-time price updates with WebSockets (contact us on [Telegram](https://t.me/MobulaPartnerBot?start=WSS_Guide) for more details)
- Fetch historical price data with market/history endpoint (check [here](/api-reference/endpoint/get-the-market-metrics-of-an-asset-over-a-given-timeframe) for more details)
