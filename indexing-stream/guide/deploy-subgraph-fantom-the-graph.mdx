---
title: "Deploy an Indexer (Subgraph) on Fantom with The Graph"
description: "A step-by-step guide to deploying a subgraph on Fantom using The Graph for efficient blockchain data indexing."
---

Deploying a subgraph on Fantom with Mobula’s infrastructure is a simple process. This guide will take you through the steps to get your subgraph running on the Fantom network.
...
<Steps>
  <Step title="Prepare Your Subgraph">
    Make sure your subgraph is configured for Fantom. Update your `subgraph.yaml` file to point to Fantom-specific contracts and events. If you don’t have a subgraph, you can clone the Uniswap V2 (SpookySwap) subgraph template and customize it for Fantom:

    ```bash
    git clone https://github.com/MobulaFi/univ2-subgraph-fantom
    ```
  </Step>
  ...
  <Step title="Deploy to Mobula's Fantom Node">
    Use the following command to deploy your subgraph:

    ```bash
    graph deploy --node https://subgraph.mobula.io:8020 --ipfs https://subgraph.mobula.io:5001 --network fantom
    ```
  </Step>
...
- Keep your subgraph regularly updated to ensure it remains compatible with the latest Fantom network changes.
