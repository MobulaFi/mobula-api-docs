---
title: "Deploy an Indexer (Subgraph) on Gnosis (xDai) with The Graph"
description: "Step-by-step guide for deploying a subgraph on Gnosis (xDai) using The Graph for efficient blockchain data indexing and querying."
---

Deploying a subgraph on Gnosis (xDai) with Mobula’s infrastructure is a simple process. This guide will take you through the steps to get your subgraph running on the Gnosis network.
...
<Steps>
  <Step title="Prepare Your Subgraph">
    Make sure your subgraph is configured for Gnosis (xDai). Update your `subgraph.yaml` file to point to Gnosis specific contracts and events. If you don’t have a subgraph, you can clone the Uniswap V2 (SushiSwap) subgraph template and customize it for Gnosis:

    ```bash
    git clone https://github.com/MobulaFi/univ2-subgraph-gnosis-xdai
    ```
  </Step>
  ...
  <Step title="Deploy to Mobula's Gnosis (xDai) Node">
    Use the following command to deploy your subgraph:

    ```bash
    graph deploy --node https://subgraph.mobula.io:8020 --ipfs https://subgraph.mobula.io:5001 --network xdai
    ```
  </Step>
...
- Keep your subgraph regularly updated to ensure it remains compatible with the latest Gnosis (xDai) network changes.
