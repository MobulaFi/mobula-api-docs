---
title: "Deploy an Indexer (Subgraph) on Alvey Chain with The Graph"
description: "Step-by-step guide for deploying a subgraph on Alvey Chain using The Graph for efficient blockchain data indexing and querying."
---

Creating a subgraph on Alvey using Mobula’s infrastructure is a straightforward task. This guide will help you set up and deploy your subgraph on the Alvey network efficiently.
...
<Steps>
  <Step title="Prepare Your Subgraph">
    Set up your subgraph for Alvey Chain. Update your `subgraph.yaml` file to reference Alvey specific contracts and events. If you don't have a subgraph, you can clone the Uniswap V2 (AlveySwap) subgraph template and adapt it for Alvey Chain:

    ```bash
    git clone https://github.com/MobulaFi/univ2-subgraph-alveychain
    ```
  </Step>
  ...
  <Step title="Deploy to Mobula's Alvey Chain Node">
    Use the following command to deploy your subgraph:

    ```bash
    graph deploy --node https://subgraph.mobula.io:8020 --ipfs https://subgraph.mobula.io:5001 --network alvey
    ```
  </Step>
...
- Keep your subgraph up to date to ensure it aligns with the latest Alvey Chain changes.
