...
<Steps>
  <Step title="Prepare Your Subgraph">
    Make sure your subgraph is configured for Match Chain. Update your `subgraph.yaml` file to point to Match Chain specific contracts and events. If you don’t have a subgraph, you can clone the Uniswap V2 (SpookySwap) subgraph template and customize it for Match Chain:

    ```bash
    git clone https://github.com/MobulaFi/univ2-subgraph-matchchain
    ```
  </Step>
  ...
  <Step title="Deploy to Mobula's Match Chain Node">
    ...
    ```bash
    graph deploy --node http://subgraph.mobula.io:8020 --ipfs http://subgraph.mobula.io:5001 --network Matchchain
    ```
  </Step>
